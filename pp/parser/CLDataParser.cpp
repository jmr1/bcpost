...
CLDataParser::CLDataParser()
{
    build_symbols();
}

void CLDataParser::build_symbols()
{
    sym.clear();
...
bool CLDataParser::simple_parse(int line, const std::string& data, std::vector<interface::AttributeVariant>& value,
                                std::string& message, bool single_line_msg)
{
    std::istringstream                                                                  input(data);
    typedef boost::spirit::classic::position_iterator2<boost::spirit::istream_iterator> pos_iterator_type;
